...
parser = argparse.ArgumentParser(
    description='Reduce embedding space for faster processing.')

parser.add_argument("--embeddings", type=str, default="", help="")
parser.add_argument("--embeddings_header",
...
parser.add_argument("--storedict_version", type=str,
                    default="countable", help="Either of continuous or countable.")
...
embeds.load(config['embeddings'], load_first_n=config['load_first_n'],
            header=config['embeddings_header'])
...
embeds.filter(set([k for k, v in Ltrain.L['original']])
              | set([k for k, v in Ltest.L['original']]))
